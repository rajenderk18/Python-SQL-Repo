...
labelPath = tk.Label(root, text='1st File: ', width = 250, bg='lightsteelblue2')
...
labelName = tk.Label(root, text='2nd File: ', width = 250, bg='lightsteelblue2')
labelName.config(font=('helvetica', 12))
canvas1.create_window(150, 160, window=labelName)

filePath2 = tk.StringVar(root)
textboxName = tk.Entry(root, textvariable = filePath2)
...
def CreateFile():
    ...
    with open(fileName.get()) as fp:
        data = fp.read()
    
    # Reading data from file2
    with open(fileName2.get()) as fp:
        data2 = fp.read()
    ...
    with open (fileName.get(), 'w') as fp:
        fp.write(data)
    ...
    with open(fileName.get(), 'r') as read_obj, open(dummy_file, 'w') as write_obj:
        ...
        todaydate2 = datetime.today().strftime('%m%d%Y')
        header = 'FH|ZIN|K627|N|' + todaydate2 + '|172001|1.0'
        ...
        trailer = 'FF|' + str_len + '|ZIN|K627|' + todaydate2 + '|172001'
        # Write given line to the dummy file
    ...
    final_output_file = "1240 - WEX Eligibility File " + str(todaydate) + ".txt"
    ...
    # # Read in the file
    # with open(str(fileName.get())[:-3] + "csv", 'r') as file :
    #     filedata = file.read()
    ...
    # with open(str(fileName.get())[:-3] + "csv", 'w') as file:
    #     file.write(filedata)
